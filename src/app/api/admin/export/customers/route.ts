...
export async function GET(req: NextRequest) {
  ...
  // Last payment amount from latest confirmed payment per user
  ...
  const rows = filtered.map(u => {
    ...
    const lastPayment = u.payments[0]?.amount ? Number(u.payments[0].amount) : 0
    return {
      id: u.id,
  ...
  if (format === 'csv') {
    const header = ['User ID','Name','Email','Phone','Membership','Status','Joined','Next Billing','Last Paid (date)','Total Paid','Last Payment']
